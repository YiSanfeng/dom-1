window.dom = {
    ...
    text (node, string) {
        if (arguments.length === 2) node.innerText = string;
        else if (arguments.length === 1) return node.innerText;
    },
    html (node, string) {
    ...
    style (node, name, value) {
        ...
        node.style[name] = value;
            ...
            return node.style[name];
                ...
                const Object1 = name;
                for(let key in name)node.style[key] = Object1[key];
            }
        }
    },
    class: {
    ...
    next (node) {
        ...
        while(x && x.nodeType === 3)x = x.nexSibling;
        return x;
    },
    previous (node) {
